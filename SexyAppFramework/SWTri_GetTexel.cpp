...
#else
{
	int umid = u - 0x8000;
	int vmid = v - 0x8000;
	int umidfloor = max(0, min(FixedFloor(umid), uMask));
	int vmidfloor = max(0, min(FixedFloor(vmid), vMask));
	...
	unsigned int rb = (((t00 & 0xFF00FF) * a00 + (t10 & 0xFF00FF) * a10 + (t01 & 0xFF00FF) * a01 + (t11 & 0xFF00FF) * a11) >> 8) & 0xFF00FF;
	unsigned int g = (((t00 & 0xFF00) * a00 + (t10 & 0xFF00) * a10 + (t01 & 0xFF00) * a01 + (t11 & 0xFF00) * a11) >> 8) & 0xFF00;
