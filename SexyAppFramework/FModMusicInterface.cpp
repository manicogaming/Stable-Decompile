...
FSOUND_SAMPLE* FModMusicInterface::LoadFMODSample(const SexyString& theFileName)
{
	SexyString aFilename = theFileName;

	int aLastDotPos = aFilename.rfind(_S('.'));
	int aLastSlashPos = max((int) aFilename.rfind(_S('\\')), (int) aFilename.rfind(_S('/')));	
	if (aLastSlashPos < 0)
		aLastSlashPos = 0;

	SexyString aCachedName = GetAppDataFolder() + _S("cached\\") + GetFileName(theFileName, true) + _S(".wav");
	...
	if (aSample == NULL)			
	{
		aSample = gFMod->FSOUND_Sample_Load(FSOUND_FREE, (theFileName + _S(".ogg")).c_str(), 0, 0);
		...
		if (aSample != NULL)
		{
			...
			{
				FILE* fp = sexyfopen(aCachedName.c_str(), _S("wb"));
...
bool FModMusicInterface::LoadSample(int theSongId, const SexyString& theFileName, bool repeat)
{
...
bool FModMusicInterface::LoadSample(int theSongId, const SexyString& theIntroFileName, const SexyString& theRepeatFileName, bool repeat)
{
...
bool FModMusicInterface::LoadMusic(int theSongId, const SexyString& theFileName)
{
