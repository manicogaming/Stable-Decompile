...
TodStringListFormat::TodStringListFormat(const SexyChar* theFormatName, Font** theFont, const Color& theColor, int theLineSpacingOffset, unsigned int theFormatFlags) :
...
{
	const SexyChar* aNameStart = sexystrchr(thePtr, L'[');
	if (aNameStart == nullptr)  // 如果文本中不存在“[”
	{
		if (sexystrspn(thePtr, _S(" \n\r\t")) != sexystrlen(thePtr))  // 如果文本不全是空白字符
		{
	...
	{
		const SexyChar* aNameEnd = sexystrchr(aNameStart + 1, L']');
...
void TodStringRemoveReturnChars(SexyString& theString)
{
	...
	{
		if (theString[i] == _S('\r'))
			theString.replace(i, 1, _S(""), 0);  // 原版中此处的“1”和“""”已内联至函数内部
		else
			i++;
	}
}
...
{
	const SexyChar* aValueEnd = sexystrchr(thePtr, _S('['));
	int aLen = aValueEnd ? aValueEnd - thePtr : sexystrlen(thePtr);
	theValue = Sexy::Trim(SexyString(thePtr, aLen));  // 如果存在下一个“[”，则取到“[”前为止；否则，取剩下的全部
...
void TodWriteStringSetFormat(const SexyChar* theFormat, TodStringListFormat& theCurrentFormat)
{
	for (int i = 0; i < gTodStringFormatCount; i++)
	{
		...
		if (sexystrncmp(theFormat, aFormat.mFormatName, sexystrlen(aFormat.mFormatName)) == 0)
		{
...
{
	return theChar == _S(' ') || (TestBit(theCurrentFormat.mFormatFlags, TodStringFormatFlag::TOD_FORMAT_IGNORE_NEWLINES) && theChar == _S('\n'));
}
